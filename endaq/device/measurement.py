...
__all__ = ['ANY', 'ACCELERATION', 'ALTITUDE', 'ANG_RATE', 'AUDIO', 'DIRECTION',
           ...
           'get_measurement_type', 'filter_channels']
...
class MeasurementType:
    ...
    def __repr__(self):
        if self.verbose:
            return "<Measurement Type: {self._name} {self.key!r})>".format(self=self)
        return "<Measurement Type: {self._name}>".format(self=self)
    ...
    def match(self, channel):
        ...
        if isinstance(channel, Channel):
            ...
            raise TypeError("Cannot compare measurement types with {} ({})".format(channel, type(channel)))
...
def split_types(query):
    ...
    for token in shlex(query):
        if token in MeasurementType.types:
            ...
            raise TypeError("Unknown measurement type: {!r}".format(token))
        prev = token
    return inc, exc
